...
public final class MemcachedCommandDecoder extends FrameDecoder {
	...
	private Object processLine(List<ChannelBuffer> parts, Channel channel, ChannelHandlerContext channelHandlerContext) throws UnknownCommandException, MalformedCommandException {
		...
		try {
			...
			if (op == null) throw new IllegalArgumentException("unknown operation: " + parts.get(0).toString());
		...
		case DELETE:
			...
			if (numParts >= MIN_BYTES_LINE) {
				if (parts.get(numParts - 1).equals(NOREPLY)) {
					cmd.noreply = true;
					if (numParts == 4) cmd.time = BufferUtils.atoi(parts.get(MIN_BYTES_LINE));
				} else if (numParts == 3) cmd.time = BufferUtils.atoi(parts.get(MIN_BYTES_LINE));
			}
		...
		case INCR:
			// Malformed
			if (numParts < MIN_BYTES_LINE || numParts > 3) throw new MalformedCommandException("invalid increment command");
		...
		case FLUSH_ALL:
			if (numParts >= 1) {
				if (parts.get(numParts - 1).equals(NOREPLY)) {
					cmd.noreply = true;
					if (numParts == 3) cmd.time = BufferUtils.atoi((parts.get(1)));
				} else if (numParts == MIN_BYTES_LINE) cmd.time = BufferUtils.atoi((parts.get(1)));
			}
			return cmd;
		case VERBOSITY: // verbosity <time> [noreply]\r\n
			// Malformed
			if (numParts < MIN_BYTES_LINE || numParts > 3) throw new MalformedCommandException("invalid verbosity command");
			
			cmd.time = BufferUtils.atoi((parts.get(1))); // verbose level
			
			if (numParts > 1 && parts.get(MIN_BYTES_LINE).equals(NOREPLY)) cmd.noreply = true;
		...
		case CAS:
			...
			if (numParts > 5) {
				...
				if (numParts == noreply + 1 && parts.get(noreply).equals(NOREPLY)) cmd.noreply = true;
			}
